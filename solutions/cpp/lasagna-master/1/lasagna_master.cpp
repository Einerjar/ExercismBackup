...
int preparationTime(const vector<string>& layers, int prep_time_per_min) {
	int num_layers = layers.size();
	return num_layers * prep_time_per_min;
}

int preparationTime(const vector<string>& layers) {
	int num_layers = layers.size();
	return num_layers * 2;
}
...
vector<double> scaleRecipe(const vector<double>& two_portion_amts, int num_portions) {
	double scaling = num_portions / 2;
	vector<double> new_recipe;

	for (int idx = 0; idx < two_portion_amts.size(); idx++) {
