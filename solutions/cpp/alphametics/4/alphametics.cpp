...
namespace alphametics {
	...
	DFSNode::DFSNode(unordered_map<char, int> new_vals, set<int> new_used) {
		current_vals = new_vals;
		used = new_used;
	}

	std::vector<DFSNode> DFSNode::successors(unordered_map<char, vector<int>> candidates) {
	...
	void populate_coeffs(unordered_map<char, int>& coeffs, string substr, bool is_lhs) {
		int multiplier = 1;
		int sign;
...
// TODO: add your solution here
	optional<unordered_map<char, int>> solve(string input) {
		...
		unordered_map<char, int> initial, coeffs, leaders;
		...
		//unordered_set<unordered_map<char, int>> visited;
