...
namespace alphametics {
	...
	optional<unordered_map<char, int>> solve(string input) {
		...
		unordered_map<char, int> leaders, coeffs;
		...
		for (const auto& pair : coeffs) {
			...
			vals_in_order.emplace_back(pair.second);
		}
		...
		while (next_permutation(digits.begin(), digits.end())) {
			...
			for (int i = 0; i < num_coeffs; i++) {
				...
				auto digit_at = digits[i];
				if (digit_at == 0 && leaders.count(char_at)) {
					leading_zero = true;
					break;
				}
				value += vals_in_order[i] * digit_at;
			}
